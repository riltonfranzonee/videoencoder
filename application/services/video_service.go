...
func (v *VideoService) Finish() error {
	...
	if err != nil {
		log.Println("Failed to delete mp4 ", v.Video.ID+".mp4")
		return err
	}
	err = os.Remove(os.Getenv("LOCAL_STORAGE_PATH") + "/" + v.Video.ID + ".frag")
	if err != nil {
		log.Println("Failed to delete frag ", v.Video.ID+".frag")
		return err
	}
	err = os.RemoveAll(os.Getenv("LOCAL_STORAGE_PATH") + "/" + v.Video.ID)
	if err != nil {
		log.Println("Failed to delete remaining files", v.Video.ID+".mp4")
		return err
	}
